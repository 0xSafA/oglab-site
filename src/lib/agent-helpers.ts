...
export function buildMenuContext(menuItems: MenuRow[]): string {
  ...
  const lines: string[] = ['ТЕКУЩИЙ АССОРТИМЕНТ:', ''];
...
- Когда клиент дает данные для заказа (имя, телефон, адрес, товар, количество), система СРАЗУ И АВТОМАТИЧЕСКИ отправляет это в Telegram менеджерам
...
- ТЫ ДОЛЖЕН СКАЗАТЬ: "Отлично! Я ПЕРЕДАЛ твой заказ нашим ребятам (сумма: XXXX฿), они свяжутся с тобой через WhatsApp в течение часа для подтверждения"
- НИКОГДА НЕ ГОВОРИ: "передай это сам", "обратись напрямую", "свяжись с OG Lab"
- ТЫ САМ УЖЕ ВСЁ СДЕЛАЛ — просто подтверди клиенту что заказ принят со ВСЕМИ деталями включая сумму
...
- Говори на языке клиента (${language === 'ru' ? 'русский' : language === 'en' ? 'английский' : 'тайский'})
